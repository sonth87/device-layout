      ...
      {open && (
        <>
          ...
          <motion.div
            ...
            className="fixed z-[9971] bottom-12 right-4 w-[380px] bg-neutral-900/95 backdrop-blur-3xl rounded-2xl shadow-2xl border border-white/10 overflow-hidden"
          >
            {/* Header: Date + Time */}
            <div className="px-5 pt-4 pb-3 border-b border-white/8">
              ...
              <p className="text-[12px] text-white/50 mt-0.5">{now.toLocaleDateString('en-US', { weekday: 'long', month: 'long', day: 'numeric' })}</p>
            </div>
              ...
              <div className="grid grid-cols-3 gap-2">
                {toggles.map((t) => (
                  <button
                    ...
                    className={cn(
                      'flex flex-col items-start gap-1.5 p-3 rounded-xl transition-colors text-left',
                      t.active ? 'bg-blue-500/30 border border-blue-400/40' : 'bg-white/8 border border-white/5 hover:bg-white/12'
                    )}
                  >
                    <div className={cn('w-6 h-6 flex items-center justify-center', t.active ? 'text-blue-300' : 'text-white/60')}>{t.icon}</div>
                    <span className={cn('text-[10px] font-medium', t.active ? 'text-white/90' : 'text-white/50')}>{t.label}</span>
                  </button>
                ))}
              </div>
            </div>
            ...
            {/* Calendar */}
            <div className="px-5 py-3 border-b border-white/8">
              <p className="text-[11px] font-semibold text-white/50 mb-2">{monthName}</p>
              <div className="grid grid-cols-7 gap-0.5 text-center">
                {['S','M','T','W','T','F','S'].map((d, i) => (
                  <div key={i} className="text-[9px] text-white/30 pb-1">{d}</div>
                ))}
                {days.map((d, i) => (
                  <div key={i} className={cn(
                    'text-[11px] w-7 h-7 flex items-center justify-center rounded-full mx-auto',
                    d === now.getDate() ? 'bg-blue-500 text-white font-semibold' : d ? 'text-white/70 hover:bg-white/10 cursor-default' : ''
                  )}>
                    {d}
                  </div>
                ))}
            ...
            {unread.length > 0 && (
              <div className="px-5 py-3 max-h-36 overflow-y-auto">
                <p className="text-[11px] font-semibold text-white/50 mb-2">Notifications</p>
                {unread.slice(0, 4).map((n) => (
                  <div key={n.id} className="flex items-start gap-2.5 py-1.5 group">
                    ...
                    <div className="flex-1 min-w-0">
                      <p className="text-[11px] text-white/90 font-medium truncate">{n.title}</p>
                      <p className="text-[10px] text-white/40 truncate">{n.body}</p>
                    </div>
...
function SliderRow({ icon, defaultValue }: { icon: React.ReactNode; defaultValue: number }) {
  ...
  return (
    <div className="flex items-center gap-2.5">
      <span className="text-white/50 w-4 shrink-0">{icon}</span>
      <input
        type="range" min={0} max={100} value={value}
        onChange={(e) => setValue(Number(e.target.value))}
        className="flex-1 h-1 rounded-full accent-blue-500"
      />
      <span className="text-[10px] text-white/40 w-7 text-right">{value}%</span>
    </div>
  );
}
