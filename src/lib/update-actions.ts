...
/**
 * Lets a host app (e.g. Electron) supply real OTA-update status + a native
 * file picker for offline/manual updates. device-layout has no IPC/network
 * access itself (ports & adapters, environment-agnostic) — the "Update"
 * Settings section hides its status/action rows when this isn't provided
 * (mirrors wallpaper-import.ts's pattern).
 */
const UpdateActionsContext = createContext<UpdateActions | null>(null);

export const UpdateActionsProvider = UpdateActionsContext.Provider;

export function useUpdateActions(): UpdateActions | null {
  return useContext(UpdateActionsContext);
}
